node {
    ...
    env.IMAGE = 'jnnjeude/amazon'

    stage('Clone repository') {
             git branch: 'main', url: 'https://github.com/jnnjeude/argocd-amazon-manifest.git'  
    }
            ...
            script {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    withCredentials([usernamePassword(credentialsId: 'jenkins-github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        ...
                        //script  {def IMAGE='jnnjeude/amazon'}
                        sh "git config user.email njeudeuj@gmail.com"
                        sh "git config user.name jnnjeude"
